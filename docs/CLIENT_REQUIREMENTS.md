...
| 14 | **Registration / onboarding model.** The merged frontend registration UI collects username + gender + date of birth + email and says "Password will be sent to this mail"; the approved backend `RegisterDto` requires username + **password** + (email OR phone) and rejects unknown fields. Who chooses the password (player-entered vs. system-generated and delivered)? Is email/SMS delivery in scope (no SMTP/SES/SMS exists)? Are gender / date of birth (18+ check) required, stored, and where? Or are player accounts operator-provisioned only (points-only platform)? | Auth / Product | Registration screen + `POST /auth/register` contract, possible new DB columns, any email/SMS infrastructure | **Registration is deliberately NOT connected** (frontend returns a user-safe "not available yet" message and sends nothing). See MEMORY.md 2026-09-24. |
