...
12. **Frontend ↔ backend integration (2026-09-24, branch `fix/frontend-backend-integration`).** Login/session/logout/lobby now use the real Phase 2B backend via one API client (`apps/web/src/services/api/client.ts`); demo auth removed; merge-marker artifacts in `package-lock.json` and `MEMORY.md` resolved. **Registration is intentionally NOT connected** — UI (username, gender, dateOfBirth, email, "password will be sent to this mail") conflicts with backend `RegisterDto` (password required; unknown fields rejected); business decision required — see `docs/CLIENT_REQUIREMENTS.md` item 14 and MEMORY.md 2026-09-24. Also open: game pages still use the mock simulator (PINTU/hardcoded balance) until Step 7; login art shows "Khelo Jeeto" branding and "100 FREE CHIPS on every login" copy unsupported by the backend; `changePassword.ts` still a stub; `rounds.integration.spec.ts` test 10 can flake when run in parallel with the points suite (global `points_transactions` count) — run integration suites with `--no-file-parallelism`.
